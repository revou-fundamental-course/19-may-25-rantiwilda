function createTemperatureConverter(parentId, isCelsiusToFar) {
  ...
  const direction = isCelsiusToFar
    ? "Celsius ke Fahrenheit"
    : "Fahrenheit ke Celsius";
  const inputLabel = isCelsiusToFar
    ? "Celsius (&deg;C)"
    : "Fahrenheit (&deg;F)";
  const outputLabel = isCelsiusToFar
    ? "Fahrenheit (&deg;F)"
    : "Celsius (&deg;C)";
  ...
  const convertFunction = isCelsiusToFar
    ? "convertToFahrenheit"
    : "convertToCelsius";
...
function validateInput(input) {
  ...
  if (input.value === "") {
    ...
    const fahrenheitValue = isCelsius
      ? (temperatureValue * 9) / 5 + 32
      : temperatureValue;

    if (
      (isCelsius && temperatureValue < 10) ||
      (!isCelsius && fahrenheitValue < 50)
    ) {
      input.closest(".col").querySelector(".emoji").textContent = "🥶";
    } else if (
      (isCelsius && temperatureValue < 17) ||
      (!isCelsius && fahrenheitValue < 63)
    ) {
      input.closest(".col").querySelector(".emoji").textContent = "😊";
    } else if (
      (isCelsius && temperatureValue < 25) ||
      (!isCelsius && fahrenheitValue < 77)
    ) {
      input.closest(".col").querySelector(".emoji").textContent = "😄";
    } else if (
      (isCelsius && temperatureValue < 35) ||
      (!isCelsius && fahrenheitValue < 95)
    ) {
      input.closest(".col").querySelector(".emoji").textContent = "😅";
    } else if (
      (isCelsius && temperatureValue <= 50) ||
      (!isCelsius && fahrenheitValue <= 122)
    ) {
      input.closest(".col").querySelector(".emoji").textContent = "😰";
    } else if (
      (isCelsius && temperatureValue > 50) ||
...
function convertTemperature(
  ...
  const outputValue = isToFahrenheit
    ? (inputValue * 9) / 5 + 32
    : ((inputValue - 32) * 5) / 9;
  const formattedOutput = parseFloat(outputValue.toFixed(3));

  document.getElementById(outputElementId).value = formattedOutput;
  document.getElementById(formulaElementId).value = isToFahrenheit
    ? `(${inputValue}°C × 9/5) + 32 = ${formattedOutput}°F`
    : `(${inputValue}°F − 32) × 5/9 = ${formattedOutput}°C`;
}
...
function reset() {
  const elements = document.querySelectorAll(
    "#inputC, #inputF, #calcMethod, #outputC, #outputF, #calcMethod2"
  );
...
function restart() {
  const elements = document.querySelectorAll(
    "#outputF, #calcMethod, #outputC, #calcMethod2"
  );
  elements.forEach((element) => (element.value = ""));
}
